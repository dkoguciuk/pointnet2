...
import re
r = re.compile('model_\d+.ckpt')
models = np.unique([r.match(m).group() for m in os.listdir(LOG_DIR) if r.match(m) is not None])
model_name = 'model_1.ckpt'
if models is not None:
    model_max = np.max([int(m[6:m.find('.')]) for m in models])
    model_name = 'model_' + str(model_max + 1) + '.ckpt'
